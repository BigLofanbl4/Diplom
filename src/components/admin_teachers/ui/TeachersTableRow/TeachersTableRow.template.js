export const TeacherRow = (teacher) => {
  ...
  const groups = Array.isArray(teacher.groups)
    ? teacher.groups.map(g => `<a href="/admin/groups/${g.id}">${g.group_number}</a>`).join(",")
    : null;
  const groupsLabel = groups || `${teacher.groups_count ?? 0} групп`;
  return `
    ...
    <td class="table__col table__medium-col" data-label="Группы">${groupsLabel}</td>
